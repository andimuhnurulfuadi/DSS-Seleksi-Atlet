...
<!DOCTYPE html>
 ...
 if(!ctype_alnum($user) OR !ctype_alnum($pass)){
   ...
   $kdkt = $_POST['kriteria'];
   $nama=$_POST['nama'];
   $querySimpan= mysqli_query($koneksi,"INSERT INTO tbl_sub_kriteria VALUES ('$kd','$kdcabor','$kdkt','$nama')");
   ...
   if($querySimpan){
     ...
     echo "<script type='text/javascript'>
         ...
         function(isConfirm) {
           if (isConfirm) {
             window.location='$admin_url'+'adminweb.php?module=tambah_subkriteria';
           }
         });}, 100);</script>";
   }
 }
